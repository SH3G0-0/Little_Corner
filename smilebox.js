    ...
    if(!document.getElementById('sb-overlay')) {
        const html = `
            <div id="sb-overlay">
                ...
                <div id="sb-journal-zone"><div id="sb-thinking">Hmm...</div></div>
                <div id="sb-letter-view" onclick="window.closeSmileBox()">
                    ...
                    <div style="text-align: center; margin-top: 50px; opacity: 0.4; font-family: 'Quicksand', sans-serif;">( Click anywhere to close )</div>
                </div>
            </div>
        `;
        ...
        document.getElementById('sb-overlay').addEventListener('click', function(e) {
            if (e.target.id === 'sb-overlay' || e.target.id === 'sb-journal-zone') { window.closeSmileBox(); }
        });
        document.getElementById('sb-letter-view').addEventListener('click', function(e) { window.closeSmileBox(); });
    }
};
...
window.spawnParticles = function() {
    ...
    setInterval(() => {
        ...
        p.style.position = 'absolute'; p.style.left = Math.random() * 100 + 'vw';
        p.style.top = '100vh'; p.style.fontSize = (Math.random() * 15 + 10) + 'px';
        p.style.opacity = '0.6'; p.style.transition = 'all 8s linear'; p.style.filter = 'drop-shadow(0 0 10px rgba(255,255,255,0.8))';
        container.appendChild(p);
        setTimeout(() => { p.style.top = '-10vh'; p.style.transform = `translateX(${(Math.random() - 0.5) * 200}px) rotate(${Math.random() * 360}deg)`; p.style.opacity = '0'; }, 50);
...
window.startSmileBox = function() {
    ...
    setTimeout(() => { window.renderJournalPage('start'); }, 1200);
};

window.renderJournalPage = function(nodeId) {
    const node = sbStory[nodeId];
    if(!node) return;
    if(node.theme) { document.getElementById('sb-overlay').className = `theme-${node.theme}`; }
    ...
    let html = `<div class="j-corner-tl">❀</div><div class="j-corner-br">🌸</div>${heartSVG}<div class="j-greeting">${node.greeting || "Hmm..."}</div><div class="j-text">${text}</div>`;
    
    if(node.buttons) {
        html += `<div class="sb-options">`;
        node.buttons.forEach((btn, idx) => {
            html += `<div class="magic-tag" style="animation-delay: ${idx * 0.15}s;" onclick="window.selectSbOption(this, '${btn.action}', '${btn.endingLetter}', '${btn.theme}')"><div class="tag-icon">${btn.icon}</div><div>${btn.text}</div></div>`;
        });
        html += `</div>`;
    } else {
        html += `<div style="text-align:center; opacity:0.4; font-size:0.95rem; margin-top:30px; font-family:'Quicksand', sans-serif;">( Click anywhere to continue )</div>`;
        page.style.cursor = 'pointer';
        page.onclick = function() {
            if(page.classList.contains('locked')) return;
            page.classList.add('locked'); page.classList.remove('active'); page.classList.add('exiting');
            const thinking = document.getElementById('sb-thinking');
            thinking.innerText = "Hmm..."; thinking.style.opacity = '1';
            setTimeout(() => {
                page.remove(); thinking.style.opacity = '0';
                setTimeout(() => {
                    if(node.endingLetter) { window.showSbLetter(node.endingLetter); } else { window.closeSmileBox(); }
                }, 300);
            }, 800);
        };
    }
...
window.selectSbOption = function(element, nextAction, endingLetter, themeOverride) {
    ...
    element.parentElement.classList.add('locked'); element.classList.add('selected');
    ...
    setTimeout(() => {
        ...
        currentPage.classList.remove('active'); currentPage.classList.add('exiting');
        const thinking = document.getElementById('sb-thinking');
        ...
        setTimeout(() => {
            currentPage.remove(); thinking.style.opacity = '0';
            setTimeout(() => {
                if(endingLetter && endingLetter !== 'undefined') { window.showSbLetter(endingLetter); } 
                else if(nextAction && nextAction !== 'undefined') { window.renderJournalPage(nextAction); } 
                else { window.closeSmileBox(); }
            }, 300);
        }, 800);
    }, 500);
};
...
window.showSbLetter = function(letterId) {
    ...
    thinking.style.opacity = '1'; thinking.style.fontSize = "1.5rem"; thinking.style.textAlign = "center";
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            const randEnd = ["I'm really glad you came here today.", "If today still feels heavy after this... call me.", "You're my favorite notification.", "Don't disappear after this. I still want to hear about your day.", "If you haven't eaten yet, I'm about to become very annoying.", "Go look in the mirror for a second. Yep, still adorable.", "This website will always be here. More importantly, so will I.", "I hope you're smiling at least a little bit now.", "You've got this. And I've got you.", "I'm sending you a hug right now. A really long one."][Math.floor(Math.random() * 10)];
