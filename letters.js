...
try {
    ...
    // --- 4. HTML Injection Engine (WITH ABSOLUTE DOM CONTENT LOADED FAILSAFE) ---
    window.injectHTML = function() {
        ...
        // Failsafe Target: Try body first, if null (impossible due to listener), fallback to HTML element.
        const target = document.body || document.documentElement;
        target.insertAdjacentHTML('beforeend', engineHTML);
    ...
    window.openLetters = function() {
        ...
        // Double Failsafe: If HTML is completely missing when clicked, inject it immediately!
        if (!overlay || !grid) {
            window.injectHTML();
        ...
        // Triple Failsafe: If it STILL fails, don't trap the user. Break out completely.
        if (!overlay || !grid) {
            console.error("Critical Failure: HTML could not be generated.");
            return;
        }
    ...
    window.openEnvelope = function(id, element) {
        try {
            ...
            // --- FLAWLESS RAIN LOOP & INCREASED FADE ENGINE ---
            const rainMusic = document.getElementById('sfx-rain');
            if (rainMusic) {
                // Attach loop ONLY ONCE. This jumps back slightly before the file ends to skip the silent gap
                ...
                // If it's already playing nicely, just bump the volume back up
                if (rainMusic.paused || rainMusic.volume === 0) {
                    ...
                    let rainFadeIn = setInterval(() => {
                        if (rainMusic.volume < 0.35) rainMusic.volume = Math.min(rainMusic.volume + 0.02, 0.35); // BUMPED TO 0.35
                        else clearInterval(rainFadeIn);
                    }, 100);
                } else {
                    rainMusic.volume = 0.35; // BUMPED TO 0.35
                }
            }
            ...
            setTimeout(() => {
                try {
                    ...
                    console.error("Error setting up letter inside timeout: ", innerError);
                }
            }, 1200); 
    ...
    window.foldLetter = function() {
        ...
        if(rainMusic) {
            let rainFadeOut = setInterval(() => {
                if(rainMusic.volume > 0.05) rainMusic.volume = Math.max(rainMusic.volume - 0.05, 0);
