// ==========================================================
// 💌 MAGICAL LETTERS ENGINE - "The Bulletproof Edition v2"
// ==========================================================
...
try {
    ...
    letterStyles.innerHTML = `
        /* --- The Drawer Environment (Aggressive Z-Index to prevent blocking) --- */
        #drawer-overlay {
            ...
            opacity: 0; transition: opacity 0.6s ease; overflow-y: auto; padding-bottom: 100px;
        ...
        /* EMOJI IS ON THE OPPOSITE SIDE (TOP RIGHT) */
    ...
    window.injectHTML = function() {
        try {
            ...
            // Failsafe injection targeting body
            if (document.body) {
    ...
    // LAZY LOAD TRIGGER: We only inject when clicked to ensure page is 100% ready
    window.openLetters = function() {
        try {
            let overlay = document.getElementById('drawer-overlay');
            let grid = document.getElementById('envelope-grid');
            
            // Lazy load the HTML if it doesn't exist yet
            if (!overlay || !grid) {
                window.injectHTML();
            ...
            void overlay.offsetWidth; // FORCES HARDWARE TO RENDER BEFORE FADING
    ...
    window.openEnvelope = function(id, element) {
        try {
            ...
            // --- FLAWLESS RAIN LOOP & LOUDER FADE ENGINE ---
            try {
                ...
                if (rainMusic) {
                    ...
                    if (rainMusic.paused || rainMusic.volume === 0) {
                        ...
                        let rainFadeIn = setInterval(() => {
                            if (rainMusic.volume < 0.4) rainMusic.volume = Math.min(rainMusic.volume + 0.02, 0.4); // Volume bumped to 0.4
                            else clearInterval(rainFadeIn);
                        }, 100);
                    } else {
                        rainMusic.volume = 0.4;
                    }
                }
            } catch (audioErr) {
                console.log("Audio skipped to protect script flow", audioErr);
            }
            ...
            setTimeout(() => {
                try {
                    ...
                    alert("Error setting up letter: " + innerError.message);
                }
            }, 1200); 

        } catch (error) {
            alert("Error opening the envelope: " + error.message);
        }
    };
